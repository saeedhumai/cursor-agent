...
def run_tests(test_name=None):
    ...
    if (os.environ.get('ANTHROPIC_API_KEY') and 
        os.environ.get('ANTHROPIC_API_KEY').startswith('sk-')):
        print("ANTHROPIC_API_KEY present")
    else:
        print("Warning: ANTHROPIC_API_KEY environment variable not set or appears to be a placeholder")
...
async def _run_single_demo(demos_dir, demo_name, non_interactive=False):
    ...
    try:
        ...
        if hasattr(demo_module, 'main_non_interactive') and non_interactive:
            ...
            if non_interactive:
                print(f"Warning: Demo {demo_name} does not support non-interactive mode, running in normal mode")
...
def main():
    ...
    group.add_argument('--test', metavar='TEST_NAME', help='Specific test to run (e.g. "claude_agent" or "claude_agent.TestClaudeAgent")')
    group.add_argument('--demo', nargs='?', const='main', metavar='DEMO_NAME', help='Run a demo script (e.g. "chat_demo" or no value for main menu)')
    group.add_argument('--demo-list', metavar='DEMO1,DEMO2', help='Run multiple demos sequentially (comma-separated list)')
    
    # Additional options for demo mode
    parser.add_argument('--non-interactive', action='store_true', help='Run demos in non-interactive mode (skips user input prompts)')
