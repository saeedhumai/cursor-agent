...
async def main(non_interactive=False):
    ...
    try:
        ...
        print_system_message("This demo showcases the Claude agent's ability to have a conversation and answer questions.")
                            ...
                            if code_match:
                                ...
                                try:
                                    with open(factorial_file, "w") as f:
                                        f.write(code)
                                    print_system_message(f"Created factorial implementation at {factorial_file}")
                                except Exception as e:
                                    print_error(f"Error creating factorial file: {str(e)}")
                    else:
                        print_error("No response received. This may be due to an issue with tool handling.")
