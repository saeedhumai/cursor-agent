...
def is_real_api_key(api_key: str, provider: str) -> bool:
    """
...
def check_response_quality(response: str) -> bool:
    ...
    auth_error_patterns = [
        "api key", "API key", "authentication", "auth", "credential", "invalid key", 
        "expired key", "unauthorized", "not authorized", "not authenticated"
    ]
    
    for pattern in auth_error_patterns:
        if pattern in response.lower() and ("error" in response.lower() or "invalid" in response.lower()):
    ...
    if "python" in first_words and not any(word in response.lower() for word in expected_keywords["Python"]):
        print(f"Failed quality check - Python query lacks key concepts: {response[:100]}")
        return False
    
    if "file" in first_words and "open" in first_words and not any(word in response.lower() for word in expected_keywords["files"]):
        print(f"Failed quality check - file query lacks key concepts: {response[:100]}")
        return False
    
    if "test_function" in first_words and not any(word in response.lower() for word in expected_keywords["test_function"]):
