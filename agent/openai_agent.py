...
class OpenAIAgent(BaseAgent):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-4o"):
        ...
        if not self.api_key:
            raise ValueError(
                "API key must be provided either as an argument or as OPENAI_API_KEY environment variable"
            )

        # Validate API key format
        if not self._is_valid_api_key(self.api_key):
            raise ValueError(
                "The provided OpenAI API key appears to be invalid. It should start with 'sk-'."
            )
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                follow_up_response = await self.client.chat.completions.create(
                    model=self.model if self.model else "gpt-4-turbo", messages=follow_up_messages, max_tokens=4096, temperature=0.7
                )
